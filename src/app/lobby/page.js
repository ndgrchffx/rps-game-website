...
export default function LobbyPage() {
  ...
  const [challenge, setChallenge] = useState(null); // { challengerId, challengerName, challengerPoints }
  ...
  useEffect(() => {
    ...
    socket.on("game:room_created", ({ roomCode }) => {
      router.push(`/waiting?code=${roomCode}&mode=casual`);
    });
    ...
    socket.on("game:queued", ({ position }) => {
      // Sudah masuk queue, tampilkan waiting
  ...
  function joinRoom() {
    ...
    socketRef.current?.once("game:joined", ({ roomCode }) => {
      router.push(`/game?room=${roomCode}`);
    });
    socketRef.current?.once("error", (e) => setNotif(e.message));
  }
  ...
  return (
    <main style={{ minHeight:"100vh", background:"#FDF0EE", fontFamily:"inherit", paddingBottom:"100px" }}>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#fff", borderRadius:"24px", padding:"32px", maxWidth:"320px", width:"90%", textAlign:"center", animation:"fadeIn 0.3s ease" }}>
            ...
            <div style={{ display:"flex", gap:"12px" }}>
              <button onClick={() => setChallenge(null)} style={{ flex:1, padding:"12px", borderRadius:"12px", border:"2px solid #e0d0d0", background:"transparent", fontWeight:"700", color:"#888" }}>Tolak</button>
              <button onClick={acceptChallenge} style={{ flex:1, padding:"12px", borderRadius:"12px", border:"none", background:"#8B2635", color:"#fff", fontWeight:"700" }}>Terima!</button>
            </div>
          </div>
        </div>
      )}
      ...
      {notif && <div style={{ background:"#fde8e8", color:"#8B2635", padding:"10px 24px", fontSize:"13px", fontWeight:"600" }}>{notif}</div>}
      ...
      <div style={{ padding:"24px 20px", maxWidth:"680px", margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", gap:"16px", marginBottom:"32px" }}>
          <div onClick={createCasualRoom} style={{ flex:1, background:"#fff", borderRadius:"20px", padding:"28px 20px", textAlign:"center", cursor:"pointer", boxShadow:"0 2px 16px rgba(0,0,0,0.05)", transition:"transform 0.15s" }} onMouseOver={e=>e.currentTarget.style.transform="translateY(-2px)"} onMouseOut={e=>e.currentTarget.style.transform="translateY(0)"}>
          ...
          <div onClick={() => setShowJoin(!showJoin)} style={{ flex:1, background:"#fff", borderRadius:"20px", padding:"28px 20px", textAlign:"center", cursor:"pointer", boxShadow:"0 2px 16px rgba(0,0,0,0.05)", transition:"transform 0.15s" }} onMouseOver={e=>e.currentTarget.style.transform="translateY(-2px)"} onMouseOut={e=>e.currentTarget.style.transform="translateY(0)"}>
        ...
        {showJoin && (
          <div style={{ background:"#fff", borderRadius:"16px", padding:"20px", marginBottom:"24px", boxShadow:"0 2px 16px rgba(0,0,0,0.05)", animation:"fadeIn 0.2s ease" }}>
            <div style={{ display:"flex", gap:"10px" }}>
              <input value={joinCode} onChange={e=>setJoinCode(e.target.value.toUpperCase())} placeholder="Masukkan kode room (6 huruf)"
              ...
              <button onClick={joinRoom} style={{ background:"#8B2635", color:"#fff", border:"none", padding:"12px 20px", borderRadius:"10px", fontWeight:"700", fontSize:"14px" }}>JOIN</button>
            </div>
          </div>
        )}
