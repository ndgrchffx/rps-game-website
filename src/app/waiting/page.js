...
function WaitingContent() {
  ...
  if (mode === "ranked") {
    socketRef.current?.emit("game:cancel_queue");
  }
  router.push("/lobby");
}
      ...
      <div style={{ textAlign:"center", animation:"fadeIn 0.4s ease" }}>
        ...
        {mode !== "ranked" && (
          <div style={{ background:"#fff", borderRadius:"20px", padding:"28px", boxShadow:"0 4px 24px rgba(0,0,0,0.08)", marginBottom:"24px", maxWidth:"320px" }}>
            ...
            <div style={{ fontSize:"36px", fontWeight:"900", letterSpacing:"8px", color:"#8B2635", marginBottom:"20px", fontVariantNumeric:"tabular-nums" }}>{roomCode}</div>
            <button onClick={copyCode} style={{ width:"100%", padding:"12px", borderRadius:"12px", border:"2px solid #8B2635", background: copied?"#8B2635":"transparent", color: copied?"#fff":"#8B2635", fontWeight:"700", fontSize:"14px", transition:"all 0.2s" }}>
              {copied ? "✅ Tersalin!" : "📋 Salin Kode"}
            </button>
          </div>
        )}

        <div style={{ display:"flex", gap:"8px", alignItems:"center", justifyContent:"center", color:"#888", fontSize:"13px" }}>
          <div style={{ width:"8px", height:"8px", borderRadius:"50%", background:"#4CAF7D", animation:"pulse 1.5s ease infinite" }}></div>
          Menunggu pemain ke-2...
        </div>

        <button onClick={cancelQueue} style={{ marginTop:"32px", padding:"10px 24px", borderRadius:"50px", border:"none", background:"#f5eded", color:"#888", fontWeight:"700", fontSize:"13px" }}>
          ← Kembali ke Lobby
        </button>
      </div>
    </main>
